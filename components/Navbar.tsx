...
const Navbar = () => {
  ...
  useEffect(() => {
    setActiveTab(pathname === '/' ? 'Home' : pathname.slice(1).charAt(0).toUpperCase() + pathname.slice(2))
    
},[pathname])

  return (
    <nav className='sticky top-0 z-50 bg-white/95 dark:bg-gray-900/95 backdrop-blur-xl border-b border-gray-200/50 dark:border-b dark:border-gray-600/80 shadow-lg shadow-gray-900/5 dark:shadow-black/30 '>
        ...
        <div className='flex items-center justify-between h-14 sm:h-16'>
          <div className='flex items-center'>
            ...
            >
              ...
              <span className='text-sm sm:text-base md:text-lg lg:text-xl font-bold bg-gradient-to-r from-emerald-600 via-green-500 to-teal-500 bg-clip-text text-transparent'>
                <span className='hidden sm:inline '>XpenseTracker AI</span>
                <span className='sm:hidden'>XpenseTracker</span>
              </span>
            </Link>
          </div>
          <div className='hidden md:flex items-center space-x-1'>
            <Link
              href='/'
              className={
                'relative dark:text-gray-300   hover:text-emerald-600 dark:hover:text-emerald-400 px-3 lg:px-4 py-2 rounded-xl text-sm font-medium transition-all duration-200 hover:bg-emerald-50/50 dark:hover:bg-emerald-900/20 group' +
                (memoizedActiveTab === 'Home'
                  ? ' bg-emerald-50/50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400 '
                  : 'text-gray-700')
              }
              ...
              <div
                className={
                  (memoizedActiveTab === 'Home'
                    ? ' group:opacity-100'
                    : 'opacity-0') +
                  ' absolute inset-0 bg-gradient-to-r from-emerald-500/10 to-green-500/10 rounded-xl group-hover:opacity-100 transition-opacity duration-200'
                }
              ></div>
            </Link>
            <Link
          ...
          <div className='flex items-center space-x-1 sm:space-x-2'>
            ...
            <div className='hidden sm:block'>
              ...
              <SignedIn>
                <div className='p-0.5 sm:p-1 sm:w-10 sm:h-10 flex items-center justify-center rounded-lg sm:rounded-full bg-gradient-to-r from-emerald-100/50 to-green-100/50 dark:from-emerald-900/20 dark:to-green-900/20 backdrop-blur-sm border border-emerald-200/30 dark:border-emerald-700/30'>
                  <UserButton
                    appearance={{
                      elements: {
                        avatarBox:
